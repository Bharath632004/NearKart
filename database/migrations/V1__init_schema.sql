-- ============================================================
-- NearKart Flyway Migration V1 - Master Schema
-- Version: 1.0 | Author: Bharath C
-- Run by: Flyway (auto on Spring Boot startup)
-- ============================================================
...
-- ============================================================
-- TRIGGERS: Auto-update avg_rating on shops and products
-- ============================================================
...
BEGIN
    UPDATE shops
    SET avg_rating   = (SELECT COALESCE(AVG(rating), 0) FROM reviews WHERE entity_type = 'SHOP' AND entity_id = NEW.entity_id),
        total_ratings = (SELECT COUNT(*) FROM reviews WHERE entity_type = 'SHOP' AND entity_id = NEW.entity_id)
...
FOR EACH ROW
WHEN (NEW.entity_type = 'SHOP')
EXECUTE FUNCTION update_shop_avg_rating();

CREATE OR REPLACE FUNCTION update_product_avg_rating()
RETURNS TRIGGER AS $$
BEGIN
    UPDATE products
    SET avg_rating    = (SELECT COALESCE(AVG(rating), 0) FROM reviews WHERE entity_type = 'PRODUCT' AND entity_id = NEW.entity_id),
        total_ratings = (SELECT COUNT(*) FROM reviews WHERE entity_type = 'PRODUCT' AND entity_id = NEW.entity_id)
    WHERE id = NEW.entity_id;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
